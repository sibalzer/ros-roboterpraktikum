...
void InputGio::initPose()
{
    ...
    while (!ros::isShuttingDown() && isInit) 
    {
        broadcaster_.sendTransform(tf::StampedTransform(transform_, 
                ros::Time::now(), "odom_combined", "gio-start"));
...
void InputGio::handlePose(const geometry_msgs::PoseWithCovariance& pose, 
                std_msgs::Header header) {
    if (isInit)
    {
        tf::Vector3 vect_tf;
        transform_.setOrigin(tf::Vector3( pose.pose.position.x,
    ...
    try 
    {
        ...
        ROS_DEBUG("Abs: %f [m], %f [m], %f [rad]; Rel: %f [m], %f [m], %f [rad]",
            from.pose.position.x, from.pose.position.y, yaw,
            to.pose.position.x, to.pose.position.y, yaw);
