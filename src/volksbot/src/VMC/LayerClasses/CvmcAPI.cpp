...
CvmcAPI::CvmcAPI()
{
#define KENNER "VMC_API_95z"

  m_VMC_API_Version = "VMC API Version 0.95z/13.10.07";  /// mit Digitalem/analogen IO
  ...
  m_Request.push_back(CRequest(_CMDGRP_MOTOR_STATUSOUT_, _SREG_MOT_CURRENT_, DataPointers, true));
  DataPointers.clear();
  ...
  /*
    //ActualChassisTemperature
    for(i = 0; i < Channel.getMaxChannels(); i++) {
      DataPointers.push_back(&(m_VMC.Motor[i].ActualChassisTemperature));
    }
    m_Request.push_back(CRequest(_CMDGRP_MOTOR_STATUSOUT_, ?????? , DataPointers, false));
    DataPointers.clear();
  */
...
#ifdef REAL_TIME
  uint64_t ts;
  m_Trans.receiveMessage(ts);  // receive data from interface
#else
...
bool CvmcAPI::interpreteConfigurationResponse(const CMessage& Message)
{
  ...
  switch (Message.getCommand())
  {
    ...
    case _MCMD_CONTROLLER_:
      ...
      break;
    case _MCMD_USE_PWM_:
      return (m_VMC.VelocityInputType.setValue(DataFrame));
      break;
    case _MCMD_LIMITER_:
...
bool CvmcAPI::interpreteErrorMessages(const CMessage& Message)
{
  ...
  {
    case _UNKNOWN_ERR_:
      VMC_Errors.push_back(
          CError("VMC Error: unknown", "VMC_API::InterpreteErrorMessages()", ErrorString.str(), Message));
      break;
    case _PARAM_ERR_:
      VMC_Errors.push_back(
          CError("VMC Error: parameter", "VMC_API::InterpreteErrorMessages()", ErrorString.str(), Message));
      break;
    case _RANGE_ERR_:
      VMC_Errors.push_back(CError("VMC Error: parameter out of range", "VMC_API::InterpreteErrorMessages()",
