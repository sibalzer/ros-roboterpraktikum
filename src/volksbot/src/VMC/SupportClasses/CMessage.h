...
public:
  ...
  CMessage(const BYTE CommandGroup, const BYTE Command, const BYTE PacketCounter = 0)
    : m_CommandGroup(CommandGroup), m_Command(Command), m_PacketCounter(PacketCounter)
  {
    CalculateCRC();
  }

  CMessage(const BYTE CommandGroup, const BYTE Command, const std::vector<BYTE>& DataFrame,
           const BYTE PacketCounter = 0)
    : m_CommandGroup(CommandGroup), m_Command(Command), m_DataFrame(DataFrame), m_PacketCounter(PacketCounter)
  {
    CalculateCRC();
  }
