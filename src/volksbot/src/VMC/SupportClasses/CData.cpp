...
//////////////////////////////////////////////////////////////////////////////
bool CData::Init(CTranslationLayer* Trans, BYTE* pNextRequestCommand, const BYTE nCommandGroup, const BYTE nCommand,
                 const EnumDataType DataType, const CChannel& MotorChannel, const std::string& cName,
                 const double dValue, const double dMaximum, const double dMinimum, const EnumUnitPrefix UnitPrefix,
                 const EnumUnit Unit)
{
  if (m_bInitialized)
    return false;
  ...
  if (dMaximum < dMinimum || dMaximum < dValue || dMinimum > dValue)
  {
    ...
    VMC_Errors.push_back(
        CError("Maximum in not bigger than Minimum or Value is not in range", "CData::Init()", m_sName));
    return false;
  }
  m_dValue = dValue;
  m_dMaximum = dMaximum;
...
bool CData::setValue(const std::vector<BYTE>& DataFrame)
{
  ...
  switch (m_DataType)
  {
    case UnsignedChar:
      ...
      {
        VMC_Errors.push_back(
            CError("invalid data length", "CData::setValue(const std::vector<BYTE>& DataFrame)", m_sName));
        return false;
      }
