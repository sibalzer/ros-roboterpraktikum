...
#ifdef REAL_TIME
bool CTranslationLayer::receiveMessage(uint64_t& timestamp)
{
  return m_Com.receive(m_sReceiveBuffer, timestamp);
}
#else
...
bool CTranslationLayer::findMessage(CMessage& NewMessage)
{
  ...
  // TODO fix error. this function throws length_error, probably due to
  // simultaneous erase calls, one by the vmc thread, and one by the process
  // using the vmc. porting to ros, i.e. making the  vmc thread a publisher
  ...
  int t = (IndexEnd + 1);
  if (t > m_sReceiveBuffer.size())
    t = m_sReceiveBuffer.size();
  ...
  if (m_ReceivePacketCounter != NewMessage.getPacketCounter())
  {
    ...
    if (m_bReceivedMessage)
      VMC_Errors.push_back(
          CError("packet counter mismatch probably missed a message", "TRANSLATION_LAYER::findMessage()"));
  }
