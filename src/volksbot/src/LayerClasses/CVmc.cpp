...
#endif
{
  ...
  ros::Subscriber cmd_vel_sub_ = n.subscribe<geometry_msgs::Twist>(
      "cmd_vel", 10, CVcallback, ros::TransportHints().reliable().udp().maxDatagramSize(100));
  //      ros::TransportHints().reliable().tcp().tcpNoDelay(true));
  ros::Subscriber sub_limits = n.subscribe<volksbot::vel_limit>("vel_limit", 1, limitCallback);
...
int CVmc::getMotorValueLeft(int state)
{
  ...
  if ((state == MOTOR_TICKS_ABSOLUTE) || (state == MOTOR_RPM) || (state == MOTOR_PWM) ||
      (state == MOTOR_TICKS_RELATIVE))
    value *= -1;
...
void CVmc::setDigitalValues(bool value1, bool value2, bool value3)
{
  ...
  if (value1 && _digitalOut1)
    bitPattern += 1;
  if (value2 && _digitalOut2)
    bitPattern += 2;
  if (value3 && _digitalOut3)
    bitPattern += 4;
...
{
  if ((velocityLeft < -100) || (velocityLeft > 100))
    return;
  if ((velocityRight < -100) || (velocityRight > 100))
    return;
...
void CVmc::init(const char* comPort)
{
  ...
  CreateThread(0, 0, (LPTHREAD_START_ROUTINE)vmcThreadFunction, (LPVOID)this, 0, &threadId);
#else
