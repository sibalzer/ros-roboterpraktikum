...
int lms100_cola::EnableRIS(int onoff)
{
  char cmd[40];
...
int lms100_cola::SetMeanFilterParameters(int num_scans)
{
  char cmd[40];
...
int lms100_cola::SetRangeFilterParameters(float* ranges)
{
  char cmd[40];
...
int lms100_cola::EnableFilters(int filter_mask)
{
  char cmd[40];
...
unsigned char* lms100_cola::ParseIP(char* ip)
{
...
int lms100_cola::GetMACAddress(char** macaddress)
{
...
int lms100_cola::SetGateway(char* gw)
{
...
int lms100_cola::SetNetmask(char* mask)
{
...
int lms100_cola::SetPort(uint16_t port)
{
  char cmd[80];
...
int lms100_cola::SetResolutionAndFrequency(float freq, float ang_res, float angle_start, float angle_stop)
{
  ...
  char cmd[80];
  sprintf(cmd, "sMN mLMPsetscancfg %f 1 %f %f %f", freq, ang_res, angle_start, angle_stop);  // sMN mLMPsetscancfg
  SendCommand(cmd);
...
{
  char cmd[80];
  sprintf(cmd, "sMN mLMPsetscancfg %f 1 %f %f %f", 50.0, 0.5, -45.0,
          225.0);  // sMN mLMPsetscancfg  // keep sending default setup
...
void lms100_cola::ParseResult()
{
  ...
  for (unsigned int i = 118; i < strlen((char*)buffer); i++)
  {
    if (buffer[i] == ' ')
    {
      ...
      {
        measurements[ranges_count++] = static_cast<double>((strtol((const char*)buf, NULL, 16))) / 1000.0;
      }
      pos = 0;
      memset(buf, 0, 8);
    }
